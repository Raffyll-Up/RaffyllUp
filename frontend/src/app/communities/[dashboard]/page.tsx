...
export default function OrganizationPage({ params }: OrganizationPageProps) {
  ...
  const currentCommunity = communityData.find(
    (cc) => cc.id === Number(dashboardName)
  );

  useEffect(() => {
    const community = communityData[Number(dashboardName)];
      ...
      >
        <div className="flex justify-between items-center">
          <Button
            ...
            size="sm"
            onClick={() => router.back()}
            className="text-gray-400 hover:border-white mr-4"
          >
        ...
        <TabsContent value="dashboard">
          {!isLoading && currentCommunity && (
            <DashboardTab community={currentCommunity} />
          )}
        </TabsContent>

        <TabsContent value="raffles">
          {!isLoading && currentCommunity && (
            <RafflesTab community={currentCommunity} />
          )}
        </TabsContent>

        <TabsContent value="participants">
          {!isLoading && currentCommunity && (
            <ParticipantsTab community={currentCommunity} />
          )}
        </TabsContent>

        <TabsContent value="funds">
          {!isLoading && currentCommunity && (
            <FundsTab community={currentCommunity} />
          )}
        </TabsContent>

        <TabsContent value="settings">
          {!isLoading && currentCommunity && (
            <SettingsTab community={currentCommunity} />
          )}
        </TabsContent>
      </Tabs>
    </div>
